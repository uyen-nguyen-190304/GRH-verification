...
map<int, int> load_kronecker(const string& filename) {
    ...
    while (getline(infile, line)) {
        ...
        if (!(iss >> n >> chi_d_val)) {
            ...
            // ? Might throw a warning here for invalid lines
            continue;
        }
...
int main(int argc, char* argv[]) {
    // TODO: Replace these with actual file read-ins
    

    // RH inequality test
    double lhs = 2.0 * iota(eta) + C_Z(intervals);
    double rhs = 0.5 * log(static_cast<double>(d) / (M_PI * exp(EULER_CONSTANT))) + logarithmic_derivative(-1, K, chi_d_arr, lambda_arr);
